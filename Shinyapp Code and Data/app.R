...
basicdata<- alldata %>%
  ...
  filter(!is.na(EnergyConsump), !is.na(min_stop_dist), !is.na(state), !is.na(county), !is.na(system), !is.na(op_me)) %>%
...
keeps <- c("year", "state", "county", "city", "system", "EnergyConsump", "LnEnergy","min_stop_dist", "Treat", "Lat", "Long","MilesEnergized", "LnMile","LnFarm","LnFarmpop", "LnElectrifarm","Ratio_Croplivestock", "percent_white", "dist_tolargecity", "AvgDcompB", "AvgRcompB")
...
server <- shinyServer(function(input, output, session) {
  ...
  readata <- reactive({
    ...
    validate(need(input$miny < input$maxy, label=strong("Error: Start year should be earlier than end year.")))
    psdata %>%
      select(system, year, Lat, Long, state, county, city,  EnergyConsump, min_stop_dist, MilesEnergized) %>%
  ...
  observe({
    ...
    output$histEnergy <- renderPlot({
      ...
      hist(MapsInBounds()$energy_growth,
           breaks = Breaks,
           main = "Change of Energy Consumption (Visible)",
           xlab = "KWH (%)",
  ...
  session$onFlushed(once=TRUE, function() {
    paintObs <- observe({
      ...
      if (colorBy == "Treat") {
        ...
        pal <- colorBin("Set1", colorData, 5, pretty = FALSE)
      }
  ...
  output$systable <- DT::renderDataTable({
    df <- basicdata %>%
      ...
      mutate(Dist_to_RoadshowStop= round(min_stop_dist,2),
             Dist_to_Largecity= round(dist_tolargecity,2)) %>%
      rename(MilesEnergized= op_me,
             Consumers_Per_Mile= op_cpm,
             Revenue_Per_Mile= op_rpm,
             Allotment_Allpurpose= allo_all,
             KWH_Purchasefrom_Plant= op_kwh_p,
             KWH_Billed= op_kwh_b,
             Totalcost_Purchsedpower= re_cpp) %>%
      select(year,system:EnergyConsump,Dist_to_RoadshowStop:Dist_to_Largecity,MilesEnergized, Consumers_Per_Mile, Revenue_Per_Mile, Allotment_Allpurpose, KWH_Purchasefrom_Plant, KWH_Billed, Totalcost_Purchsedpower) %>%
      arrange(system, year)
  ...
  output$downloadData <- downloadHandler(
    filename = function() {
      paste("REA_Selected_Data", ".csv", sep = "")
    },
...
ui <- shinyUI(navbarPage("REA Data", id="nav",
                         ...
                         tabPanel("Interactive map",
                                  div(class="outer",
                                      ...
                                      leafletOutput("map", width="1080", height="768"),
                                      ...
                                      absolutePanel(id = "controls", class = "panel panel-default", fixed = TRUE, draggable = TRUE, top = 60, left = "auto", right = 20, bottom = "auto", width = 330, height = "auto",
                                                    ...
                                                    selectInput("color", "Point Color", vars_color),
                                                    selectInput("size", "Point Size", vars_size, selected = "energy_growth"),
                                      ...
                                      tags$div(id="cite",
                                               'Note:', '(1) energy consumption data in 1938-1940 use different measure with later years.', tags$em('Please refer to README file for details.'), tags$br(), '(2) we also create animation to show the distribution of REA borrowers and roadshow stop from 1938 to 1946, which can be found ', tags$a(href="https://ibb.co/YXVgSG7", "here")
                                      )
                                  )
                         ),
                         tabPanel("Data explorer",
                                  fluidRow(
                                    column(2,
                                           sliderInput("Y", label= h4("Data Year"), min = 1938, 
                                                       max = 1946, value = c(1941, 1946), sep="")
                                    )
                                  ),
                                  tags$h5(strong("Note: Allotment size, energy purchased amount, cost and billed are only available from 1941.")),
                                  hr(),
                                  fluidRow(
                                    column(3,
                                  ...
                                  # Download Button
                                  tags$h4(strong("Download Selected Data")),
                                  fluidRow(
                                    column(3,
                         ...
                         tabPanel("README",
                                  ...
                                  tags$h5("Thanks for guidance of project leader Victoria Plutshack, Jonathon Free, Robert Fetter. Also, thanks for project manager William Neiver and David Buch. Work is jointly accomplished by Franco Picone, Tri Truong, and Zhihao Han. Zhihao collects REA data and create this shiny app."),
